...
class FirebaseAuthController {
    ...
    async SignUp(req, res) {
        ...
        try {
            ...
            res.status(201).json({message: 'Un email de vérification a été envoyé !'});
        } catch (error) {
    ...
    async SignIn(req, res) {
        ...
        try {
            ...
            if (idToken) {
                ...
                return res.status(200).json({message: `L'utilisateur ${user.email} est connecté.`});
            } else {
    ...
    async ResetPassword(req, res) {
        ...
        try {
            ...
            res.status(200).json({message: "Email envoyé avec succès pour changer de mot de passe."});
        } catch (error) {
            const errorMessage = getFirebaseErrorMessage(error.code);
            return res.status(500).json({error: errorMessage});
        }
